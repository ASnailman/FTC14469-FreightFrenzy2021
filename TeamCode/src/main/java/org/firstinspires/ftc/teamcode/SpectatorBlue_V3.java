...
@Autonomous(name="SpectatorBlue_V3", group="MecanumDrive")
public class SpectatorBlue_V3 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            switch (programorder1) {
                ...
                case 0:
                    if (pipeline.position == SpectatorBlue_V3.BarcodeDeterminationPipeline.ShippingElementPosition.LEFT) {
                        left = true;
                        center = false;
                        right = false;
                    }
                    else if (pipeline.position == SpectatorBlue_V3.BarcodeDeterminationPipeline.ShippingElementPosition.CENTER) {
                        left = false;
                        center = true;
                        right = false;
                    }
                    else if (pipeline.position == SpectatorBlue_V3.BarcodeDeterminationPipeline.ShippingElementPosition.RIGHT) {
                        left = false;
                        center = false;
                        right = true;
                    }
                ...
                case 3:
                    if (MechDrive.GetTaskState() == Task_State.READY) {
                        MechDrive.SetTargets(180, 825, 0.5);
                    }
                    else if (MechDrive.GetTaskState() == Task_State.DONE) {
                        programorder1++;
                    }
                ...
                case 7:
                    if (MechDrive.GetTaskState() == Task_State.READY) {
                        if (left) {
                            MechDrive.SetTargets(-90, 300, 0.5);
                        }
                        else if (center) {
                ...
                case 22:
                    if (MechDrive.GetTaskState() == Task_State.READY) {
                        MechDrive.SetTargets(-80, 1400, 0.5);
                    }
                    else if (MechDrive.GetTaskState() == Task_State.DONE) {
                        programorder1++;
                    }
